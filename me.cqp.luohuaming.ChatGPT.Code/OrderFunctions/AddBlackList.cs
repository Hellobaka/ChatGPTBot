...
{
    public class AddBlackList : IOrderModel
    {
        public bool ImplementFlag { get; set; } = true;

        public int Priority { get; set; } = 100;

        public string GetOrderStr() => AppConfig.AddBlackListOrder;
        ...
        public FunctionResult Progress(CQPrivateMessageEventArgs e)
        {
            ...
            {
                sendText.MsgToSend.Add($"格式不正确: {AppConfig.AddBlackListOrder} QQ");
                return result;
            }
            if (AppConfig.BlackList.Contains(qq))
            {
                sendText.MsgToSend.Add($"目标已存在于黑名单中，目前数量: {AppConfig.BlackList.Count}");
                return result;
            }
            AppConfig.BlackList.Add(qq);
            ConfigHelper.SetConfig("BlackList", AppConfig.BlackList);
            sendText.MsgToSend.Add($"添加成功，目前黑名单数量: {AppConfig.BlackList.Count}");
            return result;
        }
    }
}
