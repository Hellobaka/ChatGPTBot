...
2. 下午的活动和任务
3. 晚上的计划和休息时间
请按照时间顺序列出具体时间点和对应的活动，用一个时间点而不是时间段来表示时间，用JSON格式返回日程表，日程请关注细节并详细描述，涉及到游戏、动漫、节目或者书籍请明确作品名称，如果是游戏需要明确写出使用的角色以及自己的心情状态
...
";
        ...
        public static List<(DateTime time, string action)> GetSchedule(string prompt)
        {
            ...
            ], Chat.Purpose.日程获取, jsonMode: false, timeout: AppConfig.ChatTimeout);
