        ...
        public override AIFunction[] GetTools()
        {
            ...
            var function = Name switch
            {
                ...
                "GetGroupChatHistory" => (Context?.EnableRecordFunction ?? false) ? AIFunctionFactory.Create(ChatRecord.GetGroupChatRecord,
                    description: "获取指定群的聊天记录。参数：groupId(long) - 群号；qq(long, 可选，默认 0) - QQ号，0 表示不限制发言人；count(int, 可选，默认 15) - 最大返回消息数量。返回值：List<ChatRecord>，按时间倒序排列的聊天记录列表。") : null,
                "GetPrivateChatHistory" => (Context?.EnableRecordFunction ?? false) ? AIFunctionFactory.Create(ChatRecord.GetPrivateChatRecord,
                    description: "获取与指定用户的私聊聊天记录。参数：qq(long) - 对方QQ号；count(int, 可选，默认 15) - 最大返回消息数量。返回值：List<ChatRecord>，按时间倒序排列的私聊记录列表，若无记录则返回空列表。") : null,
