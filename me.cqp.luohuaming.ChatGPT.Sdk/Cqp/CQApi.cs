using me.cqp.luohuaming.ChatGPT.Sdk.Cqp.Core;
...
namespace me.cqp.luohuaming.ChatGPT.Sdk.Cqp
{
    /// <summary>
    /// 表示 酷Q接口 的封装类
    /// </summary>
    public class CQApi
    {
        ...
        private static RNGCryptoServiceProvider Rng { get; set; } = new RNGCryptoServiceProvider();
        ...
        public QQMessage SendGroupMessage (long groupId, params object[] message)
        {
            ...
            try
            {
                ...
                //new CQLog(_appInfo.AuthCode).InfoSend("模拟发送", $"群:{groupId} 消息:{sendMsg}");
                //return new QQMessage(this, Next(1000, int.MaxValue - 1), sendMsg);
            }
            finally
            {
                msgHandle.Free ();
        ...
        public ChatHistoryCollection GetChatHistory(long groupId, long qq, int count)
        {
            string data = CQP.CQ_getChatHistory(this.AppInfo.AuthCode, groupId, qq, count).ToString(CQApi.DefaultEncoding);
            if (string.IsNullOrEmpty(data))
            {
                return null;
            }
            ...
            try
            {
                return new ChatHistoryCollection(this, data);
            }
            catch
            {
                return null;
        ...
        {
            string data = CQP.CQ_getChatHistoryById(this.AppInfo.AuthCode, parentId, isGroup, messageId).ToString(CQApi.DefaultEncoding);
            if (string.IsNullOrEmpty(data))
            {
                return null;
            }
